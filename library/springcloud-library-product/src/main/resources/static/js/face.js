    ...
    Init:function(){
        ...
        $(".imgBtn").click(function(){
            if(isShowImg==false){
                isShowImg=true;
                $(".faceDiv").show();
                $(this).parent().prev().animate({marginTop:"-125px",opacity:1},300);
                if($(".faceDiv").children().length==0){
                    for(var i=0;i<ImgIputHandler.facePath.length;i++){
                        $(".faceDiv").append("<img title=\""+ImgIputHandler.facePath[i].faceName+"\" src=\"../images/face/"+ImgIputHandler.facePath[i].facePath+"\" style=\"height:26px;width:26px;\">");
                    }
                    $(".faceDiv>img").click(function(){
                        isShowImg=false;
                        $(this).parent().animate({marginTop:"0px",opacity:0},300,function () {
                            $(".faceDiv").css("display","none");
                        });
                        ImgIputHandler.insertAtCursor($("#message")[0],"["+$(this).attr("title")+"]");
                    });
                }
            }else{
                isShowImg=false;
                $(this).parent().prev().animate({marginTop:"0px",opacity:0},300,function () {
                    $(".faceDiv").css("display","none");
                });
            }
        });
